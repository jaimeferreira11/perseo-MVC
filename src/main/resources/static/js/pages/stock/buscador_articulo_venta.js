...
$("#bus_cerrar").click(function(){
 ...
 function removeRegistro(){
 	var trs = $("#tablaArticulosVenta tr").length;
 	for (var i = 1; trs > i; i++) {
 		// Eliminamos la ultima columna
 		$("#tablaArticulosVenta  tr:last").remove();
 	}
 }
...
$("#bus_buscar").click(function(){
  ...
  //  error = error + inputRequired($("#bus_codigobarra"))
    ...
    //   $("#bus_codigobarra").css({'border':'1px solid #d2d6de'});
       removeRegistro();
       getArticuloByParam($("#idprecioventa").val(), $("#bus_descripcion").val())
    }
});

//obtener articulos por parametros
function getArticuloByParam(idprecioventacab, param){
    blockScreen();
    $.ajax({
        type : "GET",
        url : context + "/api/v1/articulosdeposito/venta/byParam/"+idprecioventacab+"/"+param,
...
//obtener articulos por parametros
function getArticuloByParam2(idprecioventacab, param){
            ...
            if (jsonobject.estado == 'ERROR') {
            	...
            	if(jsonobject.datos.length == 0){
                	...
                	if(jsonobject.datos.length > 1){
                		...
                		getArticuloByParam(idprecioventacab, param)
                	}else{                		
                		$.each(jsonobject.datos, function(index, element) {
                			setArticulo(element)
                		});
