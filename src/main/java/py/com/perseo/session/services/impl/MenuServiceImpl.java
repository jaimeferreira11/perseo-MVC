...
//						submenu.setIdclase(clase.getIdclase());
	...
	public Menu add(Menu entity) {
		...
		try {
			...
			logger.info("Error al insertar menu " + ex.getMessage());
			throw ex;
	...
	public Menu update(Menu entity) {
		try {
			...
			logger.info("Error al actualizar menu " + ex.getMessage());
			throw ex;
	...
	public List<Menu> getMenuByEmpresa(Integer idEmpresa, Boolean all) throws Exception {
		...
		try {
			...
			sql += " from menu a ";
			sql += " join perfil b on b.idperfil = a.idperfil ";
			sql += " where a.tipo = 'M' and b.idempresa = ?";
			if(!all){
				sql += " and a.estado = true ";
			}
			sql += " order by a.idmenu ";
